...
int main()
{ int matriz[6][6], rota[6], i, j, contadorNomeMatriz=0, contadorNomeRota=0, destino, a, b, soma = 0;
    char nomeDaMatriz[20], letraMatriz, nomeDaRota[20], letraRota;

    freopen("e4.txt", "r", stdin);
    freopen("s4.txt", "w", stdout);

    while (letraMatriz != '\n' && letraMatriz != ' ') //lendo o nome da matriz
    {
        scanf("%c", &letraMatriz);
   ...
   while (contadorN < 2) // pq ele lê dois \n, ai bota esse contador, ma nao deixar ele ler mais de um.
   {
       scanf("%c", &letraRota);
    ...
    {
        scanf("%d ", &rota[i]); //essa rota se refere ao indice que vou procurar na matriz
    }
